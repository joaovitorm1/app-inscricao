...
$('#select-edital').on("change", function () {
    ...
    if (idEdital > 0) {
        ...
        $("#select-grupoOcupacional").attr("disabled", false);

        //Carrega as opções do Tipo Cargo
        $.ajax({
            type: "GET",
            contentType: "application/json;charset=UTF-8",
        ...
        $("#select-tipoCargo").attr("disabled", false);
    ...
    if (idGrupoOcupacional > 0) {
        ...
        $.ajax({
            ...
            success: function (dados) {
                ...
                for (var i = 0; i < dados.length; i++) {
                    var requisito = '<div class="row" onclick="selectTitulo('+dados[i].id+')">'+
                                    '    <div class="form-group">'+
                                    '        <div class="custom-control custom-switch custom-switch-on-success">'+
                                    '           <input type="checkbox" name="requisitos[]" class="custom-control-input" value="'+ dados[i].id +'" id="req-'+ dados[i].id +'" >'+
                                    '           <label class="custom-control-label" for="req-'+ dados[i].id +'">'+ dados[i].nome +'</label>';
                                    '       </div>'+
                                    '   </div>'+
                                    '</div>';
        ...
        $.ajax({
            ...
            success: function (dados) {
                ...
                for (var i = 0; i < dados.length; i++) {
                    ...
                    var titulo = '<div class="row" onclick="selectRequisito('+dados[i].id+')">'+
                                    '    <div class="form-group">'+
                                    '        <div class="custom-control custom-switch custom-switch-on-success">'+
                                    '           <input type="checkbox" name="requisitos[]" class="custom-control-input" value="'+ dados[i].id +'" id="titulo-'+ dados[i].id +'" >'+
                                    '           <label class="custom-control-label" for="titulo-'+ dados[i].id +'">'+ dados[i].nome +'</label>';
                                    '       </div>'+
                                    '   </div>'+
                                    '</div>';
